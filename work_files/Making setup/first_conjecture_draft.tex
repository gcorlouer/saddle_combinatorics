...
\item if two points of $\mathcal C_{d,S}^r$ are equivalent through an element of $G_d^{W_0}$ inside $\Gamma_{d,S}^r$, then they are not identified in any genuinely new way beyond the critical locus itself.
\end{itemize}
\end{remark}
...
\mathcal C_{d,S}^r \hookrightarrow \Gamma_{d,S}^r
\]
induces a bijection on orbit spaces for the stabilizer group
\[
G_d^{W_0}
=
...
This is why the first conjecture should be stated using a group with nontrivial endpoint action, such as $G_d^{W_0}$.
