...
\]
These are the natural orbit spaces for the corrected first conjecture because $G_d^{W_0}$ can move the endpoint map while preserving the closing matrix $W_0$ that defines the cyclic equations, whereas only the smaller subgroup $H_S$ preserves the distinguished critical value $P_SW^\ast$.

\begin{remark}[A practical reformulation of the first conjecture]
The desired orbit correspondence can be phrased as follows:
\begin{quote}
Every orbit of $G_d^{W_0}$ in $\Gamma_{d,S}^r$ contains at least one point whose end-to-end map is $P_SW^\ast$, and the intersection of such an orbit with $\mathcal C_{d,S}^r$ is exactly one $H_S$-orbit.
\end{quote}
If this holds, then one obtains a bijection
\[
\Gamma_{d,S}^r / G_d^{W_0}
\cong
\mathcal C_{d,S}^r / H_S.
\]
This is exactly what is needed to transfer any $G_d^{W_0}$-orbit stratification of the cyclic locus to an $H_S$-orbit stratification of the critical locus.
\end{remark}
